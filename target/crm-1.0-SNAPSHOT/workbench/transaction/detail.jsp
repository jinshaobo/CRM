<%@ page import="com.jinshaobo.crm.settings.domain.DicValue" %>
<%@ page import="java.util.List" %>
<%@ page import="java.util.Map" %>
<%@ page import="java.util.Set" %>
<%@ page import="com.jinshaobo.crm.workbench.domain.Tran" %>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>

<%
//http://localhhost:8080/crm
...
<script type="text/javascript">
	...
	function showTranHistoryList(){
		$.ajax({
			...
			data : {
				"tranId" : "${tran.id}"//在JS中使用el表达式必须使用双引号
			},
			type : "get",
			dataType : "json",
    ...
    function changeStage(stage,i){
		...
		$.ajax({
			...
			success : function (data){
				//data{success,tran}
				 if(data.success){
					 //刷新stage,possibility,editBy,editTime
	...
	function changeIcon(stage,index1){
		...
		var index = index1;
		//前面正常阶段和后面丢失阶段的分界点下标
		var point = "<%=point%>";//java变量转换成js变量，必须是8大基本数据类型和String类型,而且必须在引号中
		/*alert("当前阶段"+currentStage);
		alert("当前阶段可能性"+currentPossibility);
		alert("当前阶段下标"+index);
		alert("前面正常阶段和后面丢失阶段的分界点下标"+point);*/

		//如果当前阶段可能性为0，说明正处于后两个阶段，则前面7个为黑圈,后两个一个红叉，一个黑叉
		...
		//如果当前阶段可能性不为0，说明正处于前7个阶段，则后面2个为黑叉，前7个不定
		}else {
			//遍历前7个
			for (var i = 0; i < point; i++) {
				//如果小于当前阶段
